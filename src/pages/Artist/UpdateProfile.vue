<template>
    <div v-if="show" class="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center">
      <div class="bg-black bg-opacity-50 rounded-lg p-3 w-full max-w-lg shadow-lg">
        ...
        <form @submit.prevent="handleFormSubmit">
          ...
          <!-- Image Upload -->
          <div class="mb-4">
            <label for="image" class="block text-white mb-2">Image</label>
          ...
          <div class="mb-4">
            ...
            <!-- Old Password -->
            <div class="mb-4">
              <label for="oldPassword" class="block text-white mb-2">Old Password</label>
              <input type="password" id="oldPassword" v-model="passwords.oldPassword" @input="clearError('oldPassword')"
                class="w-full p-2 rounded outline-none bg-gray-700 text-white focus:border-red-800 focus:ring-2 focus:ring-red-800 caret-red-800">
              <span v-if="errors.oldPassword" class="text-red-500">{{ errors.oldPassword }}</span>
            </div>
            <!-- New Password -->
            <div class="mb-4">
              <label for="newPassword" class="block text-white mb-2">New Password</label>
              <input type="password" id="newPassword" v-model="passwords.newPassword" @input="clearError('newPassword')"
                class="w-full p-2 rounded outline-none bg-gray-700 text-white focus:border-red-800 focus:ring-2 focus:ring-red-800 caret-red-800">
  ...
  const handleFormSubmit = () => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      emits('update', { ...cuser.value, passwords: passwords.value })
    } else {
  ...
  const validateForm = () => {
    ...
    if (passwords.value.newPassword && !passwords.value.oldPassword) {
      validationErrors.oldPassword = 'Old password is required to set a new password'
    }
