...
const showTrackOptions = ref({}); // For track-specific options
...
const fetchAlbumData = async () => {
  try {
    const fetchedAlbum = await fetchAlbum(albumId.value);
    album.value = fetchedAlbum;
    tracks.value = fetchedAlbum.track || [];
    artist.value = fetchedAlbum.artist;
  } catch (error) {
...
const handleAlbumEdit = async () => {
  try {
    ...
    editMode.value = false; // Exit edit mode after saving
  } catch (error) {
...
// Define the clearError function
const clearError = (field) => {
  console.log(`Clearing error for ${field}`);
  // Implement the actual error clearing logic here
};
