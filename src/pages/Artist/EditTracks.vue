<template>
    <div class="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center">
        <div class="bg-black bg-opacity-50 rounded-lg p-10 w-full max-w-lg shadow-lg border-2 border-red-800 text-left">
            <h2 class="text-2xl font-bold text-white mb-4">Edit Song</h2>
            <form @submit.prevent="handleTrackEdit">
                <div class="mb-4">
                    <label for="songTitle" class="block text-left text-white mb-2">Title</label>
                    <input type="text" id="songTitle" v-model="track.title" @input="clearError('title')"
                        class="w-full p-2 rounded outline-none bg-gray-700 border-2 border-red-800 text-black focus:border-red-800 focus:ring-2 focus:ring-red-800 caret-red-800">
