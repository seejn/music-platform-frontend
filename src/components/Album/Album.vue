...
<template>
    <div class="bg-gray-800 bg-opacity-50 rounded-lg p-4 glass-effect border-2 border-red-800 hover:shadow-lg hover:shadow-red-800 hover:border-red-800 transition-all duration-300">
        ...
        <h3 class="text-xl font-bold text-black">{{ album.title }}</h3>
        <p class="text-black">{{ album.songCount }} Songs</p>
    </div>
</template>
...
const imageUrl = computed(() => {
    if (props.album.image) return `${import.meta.env.VITE_API_BASE_URL}${props.album.image}`;
